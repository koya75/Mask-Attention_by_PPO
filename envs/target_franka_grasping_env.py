...
class FrankaGraspingEnv(VectorEnv):
    def __init__(
        ...
        if self.discrete:
            ...
            self.action_space = spaces.Box(low=-1, high=1, shape=(3, 1))#4
    ...
    def _compute_observations(self):
        ...
        self.gym.write_camera_image_to_file(self.sim, self.envs[0], self.camera.cam_handle, gymapi.IMAGE_COLOR, "camera/hand.png")"""
...
def compute_franka_reward(
    ...
    # type: (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, float, int, bool) -> Tuple[Tensor, Tensor]
    ...
    """if demo:
        object_rewards = torch.where(torch.bitwise_and(height_distance_condition, progress_buf > 10), 0.5, 0.0)
        target_rewards = torch.where(torch.bitwise_and(names, progress_buf > 10), 0.5, 0.0)
        rewards = target_rewards + object_rewards
    else:"""
    rewards = torch.where(torch.bitwise_and(names, progress_buf > 10), 1.0, 0.0)
